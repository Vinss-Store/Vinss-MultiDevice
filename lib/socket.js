...
async function initLidOwner(zassbtz) {
...
function attachSocketHelpers(zassbtz, deps) {
  ...
  zassbtz.sendButton = async (jid, content = {}, options = {}) => {
    if (!zassbtz.user?.id) throw new Error('User not authenticated');
    ...
    async function mediaHeader(kind, source) {
      ...
      return prepareWAMessageMedia(mediaInput, { upload: zassbtz.waUploadToServer });
    }
    ...
    if (image) {
      ...
      const preparedMedia = await prepareWAMessageMedia(mediaInput, { upload: zassbtz.waUploadToServer });
      ...
      messageContent.header = {
        ...
        productMessage: {
          ...
          businessOwnerJid: businessOwnerJid || product.businessOwnerJid || zassbtz.user.id,
        },
      };
    } else if (title) {
    ...
    const msg = generateWAMessageFromContent(
      ...
      { userJid: zassbtz.user.id, quoted: options?.quoted || null },
    );
    ...
    await zassbtz.relayMessage(jid, msg.message, { messageId: msg.key.id, additionalNodes });
    return msg;
  };
